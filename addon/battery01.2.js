...
//    a) init
   ...
   //   b)here   input data (like conditions)
		    ...
		    action:"level = x%",
 			...
 			{ from:{type:"xtap",
			id: "check batterie di Roma" },
			  action:"home=ROMA",
		   },
  	    	{from:{ type:"xtap",
			id: "check batterie di Milano"},
			  action:"home=MILANO",
		   },
		   {from:{ type:"extra",
		   id: "IoTwebUI"},
...
// dummy, but required because in the 'else' section!		
	...
	// test BAD user home
        if ( GETIDLIST(_showHome).length == 0){
			  myMsgBox("Battery01.2","Not found devices in "+ _showHome);
	...
	// if the attribute home, expert mode, _lowPerc changed, data cleanup
           ADDXDEVICE(xhome, xroom, xname, [{
          	...
          	VOICE("Aggiorno la lista dei device con batterie scariche di " + _showHome); // start action feedback
			REFRESH(xname);  // slow, but required to allow a loop and show the device as offline 
